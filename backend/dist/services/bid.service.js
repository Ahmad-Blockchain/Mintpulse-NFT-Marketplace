...
export class BidService {
    ...
    // Create Bid
    // ======================================================
    static async createBid(bidder, data) {
        if (!bidder) {
        ...
        if (auction.seller.toLowerCase()
            ===
                bidder.toLowerCase()) {
            throw new Error("Auction seller cannot place a bid");
        }
        // --------------------------------------------------
        // Auction Status
        // --------------------------------------------------
        ...
        // --------------------------------------------------
        const currentHighestBid = Number(auction.highest_bid ?? 0);
        const startPrice = Number(auction.start_price ?? 0);
        const minimumBid = Math.max(startPrice, currentHighestBid);
        if (data.amount <= minimumBid) {
            throw new Error(`Bid must be greater than ${minimumBid}`);
        }
        // --------------------------------------------------
        // Transaction
        // --------------------------------------------------
    ...
    // Get Bids By Auction
    // ======================================================
    static async getBidsByAuction(auctionId) {
        const [rows] = await pool.execute(`
                SELECT *
                FROM auction_bids
    ...
    // Get Bids By Bidder
    // ======================================================
    static async getBidsByBidder(bidder) {
        const [rows] = await pool.execute(`
                SELECT *
                FROM auction_bids
    ...
    static async getAllBids(query) {
        ...
        // Auction Filter
        // --------------------------------------------------
        if (query.auction_id !== undefined) {
            sql += `
                AND auction_id = ?
            `;
            values.push(query.auction_id);
        }
        // --------------------------------------------------
        // Bidder Filter
        // --------------------------------------------------
