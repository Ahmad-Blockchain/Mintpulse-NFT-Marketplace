...
// ==========================================================
export const createAuctionValidator = (req, res, next) => {
    const { nft_id, start_price, reserve_price, buy_now_price, payment_token, start_time, end_time } = req.body;
    // ------------------------------------------------------
    // NFT ID
    // ------------------------------------------------------
    if (nft_id === undefined ||
    ...
    if (reserve_price !== undefined &&
        reserve_price !== null) {
        if (Number.isNaN(Number(reserve_price)) ||
        ...
        if (Number(reserve_price) <
            Number(start_price)) {
            res.status(400).json({
                success: false,
                message: "Reserve price cannot be lower than start price"
            });
            return;
        }
    }
    ...
    if (buy_now_price !== undefined &&
        buy_now_price !== null) {
        if (Number.isNaN(Number(buy_now_price)) ||
        ...
        if (Number(buy_now_price) <
            Number(start_price)) {
            res.status(400).json({
                success: false,
                message: "Buy now price cannot be lower than start price"
            });
            return;
        }
    }
...
// ==========================================================
export const updateAuctionValidator = (req, res, next) => {
    const { reserve_price, buy_now_price, start_time, end_time } = req.body;
    if (reserve_price === undefined &&
...
// ID Validator
// ==========================================================
export const auctionIdValidator = (req, res, next) => {
...
// NFT ID Validator
// ==========================================================
export const auctionNFTIdValidator = (req, res, next) => {
